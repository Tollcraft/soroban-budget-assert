...
- `cargo budget-report --quiet` flag: suppresses non-essential stderr progress messages and warnings during execution. The final report (table, JSON, or CSV) is still printed to stdout. Fatal errors from child-process spawn failures or hard contract build failures are not suppressed.
