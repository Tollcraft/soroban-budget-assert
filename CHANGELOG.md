...
- `#[budget_scaling(…)]` attribute macro: growth-model assertion that measures CPU cost across multiple input sizes and validates that observed cost growth matches a declared model (`linear` or `quadratic`) within a configurable tolerance. Produces clear diagnostics on failure including measured sizes, costs, expected ratio, and deviation. The check uses successive ratio comparisons — no statistical curve fitting. Supports passing and failing (`#[should_panic]`) fixtures.
