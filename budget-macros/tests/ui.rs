...
fn ui_dir() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("tests").join("ui")
}
...
fn harness_covers_both_compile_fail_and_pass_fixtures() {
    assert!(!rust_fixtures(&ui_dir()).is_empty(), "no compile-fail fixtures found");
    let pass_dir = ui_dir().join("pass");
    assert!(!rust_fixtures(&pass_dir).is_empty(), "no pass fixtures found");
}
...
fn every_compile_fail_fixture_pins_its_diagnostic() {
    for fixture in rust_fixtures(&ui_dir()) {
        let snapshot = fixture.with_extension("stderr");
        assert!(snapshot.is_file(), "{}: snapshot missing", fixture.display());
        let text = read(&snapshot);
        assert!(!text.trim().is_empty(), "{}: snapshot empty", snapshot.display());
    }
}
...
fn every_pass_fixture_is_a_standalone_program() {
    for fixture in rust_fixtures(&ui_dir().join("pass")) {
        ...
        assert!(text.contains("fn main"), "{}: needs main()", fixture.display());
    }
}
...
fn missing_env_read_fixture_pins_the_missing_env_error() {
    ...
    assert!(source.contains("budget_read_bytes_lt"), "must use the read-bytes macro");
    assert!(!source.contains("env ="), "must omit the env source");
    assert!(!source.contains("env_file"), "must omit an env_file source");
    assert!(snapshot.contains("budget_read_bytes_lt"), "diagnostic must name the macro");
    assert!(snapshot.contains("E0423"), "expected rustc's E0423 for a missing env");
}
...
fn pct_out_of_range_fixture_pins_the_range_error() {
    ...
    assert!(snapshot.contains("between 1 and 100"), "diagnostic must state the range");
}
...
fn pct_no_of_fixture_pins_the_missing_of_error() {
    ...
    assert!(snapshot.contains("requires `of`"), "diagnostic must explain the missing reference");
}
