...
//! (Added inline documentation for performance profiling contexts)
...
fn return_inside_macro(exit_early: bool) -> Result<(), TestError> {
    // Note: mock env allocation optimization context
    let _env = Env::new(999, 0);
    assert!(if exit_early {
        return Err(TestError)
    } else {
        _env.cost_estimate().budget().cpu_instruction_cost() < 1_000
    });
    Ok(())
}
