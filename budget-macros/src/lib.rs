...
<<<<<<< HEAD
/// * `EnvFile` — reads `var_name` from a `KEY=VALUE` file at `path`.
=======
>>>>>>> origin/main
enum BudgetLimit {
    ...
    Config(String),
<<<<<<< HEAD
    /// A `KEY=VALUE` file path plus the key to read from that file.
    EnvFile {
...
/// Composite attribute arguments for `#[budget_lt(cpu = …, mem = …)]`.
struct BudgetSpec {
...
<<<<<<< HEAD
            // Allow trailing `, mem = …` / `, cpu = …` for `BudgetSpec`, but
            // reject mixed literal+modifier forms like `950_000, env = "…"`.
            if !input.is_empty() {
                let ahead = input.fork();
                if ahead.peek(Token![,]) {
                    let _ = ahead.parse::<Token![,]>();
                    if ahead.peek(Ident) {
                        let key = ahead.fork().parse::<Ident>().unwrap().to_string();
                        if matches!(key.as_str(), "env" | "env_file" | "config") {
...
<<<<<<< HEAD
                    if env_file.is_none() && env_var.is_none() && config_key.is_none() {
                        let ident: Ident = input.parse()?;
                        return Err(syn::Error::new(
                            ident.span(),
                            format!("expected `env` or `config`, got `{}`", ident),
                        ));
                    }
=======
>>>>>>> origin/main
...
<<<<<<< HEAD
fn limit_helpers_preamble() -> proc_macro2::TokenStream {
    quote! {
...
fn find_return_token(tokens: &proc_macro2::TokenStream) -> Option<proc_macro2::Span> {
    use proc_macro2::TokenTree;
    for tt in tokens.clone() {
        match tt {
            TokenTree::Ident(id) if id == "return" => return Some(id.span()),
...
impl VisitMut for ReturnRewriter {
    fn visit_expr_mut(&mut self, expr: &mut Expr) {
        match expr {
            Expr::Return(ret) => {
                ...
                if let Some(val) = ret.expr.as_ref() {
                    let val = val.clone();
                    *expr = parse_quote! {
                        return {
                            let __budget_returned = #val;
                            #assertion
                            __budget_returned
                        }
                    };
            ...
            Expr::Closure(_) | Expr::Async(_) => {
                // Returns inside closures / async blocks exit that
                // inner body, not the test function — leave them alone.
            }
    ...
    fn visit_item_mut(&mut self, _item: &mut syn::Item) {
        // Do not rewrite returns inside nested items.
    }
    ...
    fn visit_macro_mut(&mut self, mac: &mut syn::Macro) {
        ...
        if let Some(span) = find_return_token(&mac.tokens) {
            self.error = Some(syn::Error::new(
                span,
                "`return` inside a macro invocation is not supported by the budget macros: the assertion cannot be injected into macro tokens, so it would be skipped silently. Move the `return` out of the macro invocation, or end the test with a tail expression instead. (If this `return` belongs to a closure passed to the macro, lift the closure into a `let` binding before the macro call.)",
            ));
        }
    }
}
...
fn instrument_exit_paths(
    ...
    let mut body_stmts = std::mem::take(&mut input_fn.block.stmts);
    if let Some(Stmt::Expr(expr, None)) = body_stmts.last().cloned() {
        body_stmts.pop();
        let assertion = assertion.clone();
        let wrapped: Expr = parse_quote! {{
            let __budget_value = #expr;
            #assertion
            __budget_value
        }};
        body_stmts.push(Stmt::Expr(wrapped, None));
    } else {
        let assertion_expr: Expr = parse_quote! {{ #assertion }};
        body_stmts.push(Stmt::Expr(assertion_expr, Some(Default::default())));
    }

    let preamble_block: syn::Block = syn::parse2(quote! {{ #preamble }})?;
    let mut new_stmts = preamble_block.stmts;
...
>>>>>>> origin/main
    ...
    let preamble = limit_helpers_preamble();
    let assertion = quote! {
        let budget = #env_ident.cost_estimate().budget();
        #(#asserts)*
    };

    if let Err(e) = instrument_exit_paths(&mut input_fn, preamble, assertion) {
...
<<<<<<< HEAD
    let assert_msg_lit = syn::LitStr::new(&assert_msg, proc_macro2::Span::call_site());
=======
    ...
    proptest! {
        ...
        fn integer_attribute_parses_as_same_u64(n in any::<u64>()) {
            ...
            match parsed {
                ...
                _ => prop_assert!(false, "expected integer limit"),
            }
        }
        ...
        fn env_attribute_parses_var_name(
            ...
            match parsed {
                ...
                _ => prop_assert!(false, "expected env limit"),
            }
        }
        ...
        fn non_env_ident_with_env_syntax_is_rejected(
            ...
            prop_assume!(ident != "config");
            prop_assume!(ident != "env_file");
            let Ok(id) = syn::parse_str::<syn::Ident>(&ident) else {
                return Ok(());
            };
            let lit = syn::LitStr::new("BUDGET_LIMIT", proc_macro2::Span::call_site());
            let tokens = quote! { #id = #lit };
            prop_assert!(parse_budget_limit(tokens).is_err());
        }
    }
=======
>>>>>>> origin/main
