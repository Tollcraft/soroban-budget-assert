...
/// Discriminates between per-function measurements, per-package subtotals,
/// and workspace-wide totals. Serialized as a lowercase string so existing
/// JSON consumers can filter on it without breaking on the new field.
...
fn main() -> Result<()> {
    ...
    // Only function-level rows with a measured value contribute; aggregates
    // are never double-counted and check-failure stubs (value=None) are skipped.
    ...
    for r in &reports {
        if r.kind == BudgetKind::Function {
            if let Some(val) = r.value {
                *workspace_totals.entry(r.metric).or_insert(0) += val;
            }
        }
    }
